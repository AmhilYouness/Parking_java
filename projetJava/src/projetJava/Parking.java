...
//    tes If Exist A Place Free For New Car
    int places(){ 
...
//               If Found A Place For A Car Then Incremment Number Of Place Busy
	    ...
	    System.out.format("Parking : %s refusée, il reste  %d places \n", car.nom,places());
            goAroundTheParking(car,car.abs,car.ord);
	    return(false);
	   }
    }
...
//  Decrement Number Of The Cars In The Parking
	   ...
	   System.out.format("Parking :[%s] est sortie, reste  %d places and posY = %d\n", car.nom, places(),car.ord);   
        switch (car.ord) {
          ...
          this.moveCarByRoundOutside(car,car.abs,car.ord);
    }
...
//        Check If Palce One Empty
            if(!Place[0]){
                Place[0] = true;
                this.moveCarDirect(car,car.abs,car.ord);
            }
 //        Check If Palce Two Empty
            else if(!Place[1]){
                Place[1] = true;   
                this.moveCarByRound(car, car.abs,car.ord,160);
...
//        Check If Palce Three Empty                 
                ...
                Place[2] = true;
                this.moveCarByRound(car, car.abs,car.ord,330);
              
            }
 //        Check If Palce Four Empty     
            else if(!Place[3]){
                 Place[3] = true;
                 this.moveCarByRound(car, car.abs,car.ord,490);
